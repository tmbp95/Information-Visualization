...
function RadarChart(id, data, options) {
  ...
  for(i=0;i<data.length;i++){
    if(data[i].key == tempClickedBall){
      console.log("AQUI", data[i]);
      arrayData.push(data[i]);
    }
    if(clickedBall!=null){
      if(data[i].key == clickedBall && data[i].key != tempClickedBall){
        arrayData.push(data[i]);
      }
    }
  ...
  console.log("sdfasfadsfads",data)
